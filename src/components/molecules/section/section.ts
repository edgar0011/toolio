...
class ContentSection extends HTMLElement {
  connectedCallback() {
    ...
    const hasFloatingBg =
      this.parentElement?.querySelector('floating-lines') !== null &&
      document.documentElement.classList.contains('light')
    const sectionBg = hasFloatingBg ? '' : 'bg-t-bg'
    ...
    this.innerHTML = `
      <section class="flex items-center justify-center px-[4dvw] min-h-dvh pt-[6dvh] pb-[6dvh] snap-start ${sectionBg}">
        <div class="w-full max-w-7xl mx-auto">
          ...
          <div class="rounded-xl border border-t-border backdrop-blur-md p-[clamp(1rem,2dvw,2rem)]">
